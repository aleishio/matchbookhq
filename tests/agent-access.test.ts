...
test("agent config uses the canonical public origin instead of forwarded host", () => {
  const env = withTestEnv({
    APP_URL: "https://matchbookhq.com",
    YC_OS_ACCESS_TOKEN: "shared-test-token",
    YC_OS_UNLOCK_COOKIE_NAME: "yc_os_access_token"
  });

  try {
    const config = createAgentAccessConfig(unlockedRequest("https://www.matchbookhq.com/api/agent/sessions", {
      headers: {
        host: "evil.example",
    ...
    assert.equal(config.mcpUrl, "https://www.matchbookhq.com/api/mcp");
    assert.equal(config.capabilitiesUrl, "https://www.matchbookhq.com/api/agent/capabilities");
    assert.equal(config.config.mcpServers["yc-os"].url, "https://www.matchbookhq.com/api/mcp");
    assert.equal(config.config.mcpServers["yc-os"].headers.Authorization, "Bearer shared-test-token");
  } finally {
    env.restore();
  }
...
test("agent config forces https for matchbook host fallback", () => {
  const env = withTestEnv({
    AGENT_PUBLIC_ORIGIN: undefined,
    APP_URL: undefined,
  ...
  try {
    const config = createAgentAccessConfig(unlockedRequest("http://internal.test/api/agent/sessions", {
      headers: {
        host: "matchbookhq.com"
      }
    }));
...
test("agent config rejects untrusted public hosts without a configured origin", () => {
  const env = withTestEnv({
    AGENT_PUBLIC_ORIGIN: undefined,
    APP_URL: undefined,
    NEXTAUTH_URL: undefined,
  ...
  try {
    assert.throws(
      () => createAgentAccessConfig(unlockedRequest("http://internal.test/api/agent/sessions", {
        headers: {
          host: "evil.example"
        }
      })),
...
test("agent config keeps IPv6 localhost usable for local dev", () => {
  const env = withTestEnv({
    AGENT_PUBLIC_ORIGIN: undefined,
    APP_URL: undefined,
    NEXTAUTH_URL: undefined,
  ...
  try {
    const config = createAgentAccessConfig(unlockedRequest("http://[::1]:3300/api/agent/sessions", {
      headers: {
        host: "[::1]:3300"
      }
    }));

    assert.equal(config.mcpUrl, "http://[::1]:3300/api/mcp");
  } finally {
    env.restore();
  }
});

...
start=GET https://www.matchbookhq.com/api/agent/capabilities truth source; call /api/mcp tools/list then tools/call, or POST /api/agent/tools/call.
read_tools=get_agent_guide,list_event_prep_events,get_event_prep_context(eventId),search_founders,list_approval_events->eventId,list_approval_queue(eventId)->applicationId,get_approval_summary(applicationId),get_guest_context. Sanitized; guest context private.
